...
class CollectionService:
    ...
    async def create_collection(self, request: CollectionRequest, collection_name: str) -> Collection:
        """
        ...
        try:
            ...
            vector_store_manager = self._get_vector_store_manager(request.embeddings_model)
            ...
            logger.info(f"Collection '{collection_name}' created successfully with model '{request.embeddings_model}'")
            ...
            logger.error(f"ValueError creating collection '{collection_name}': {str(e)}")
            raise
        except Exception as e:
            logger.error(f"Unexpected error creating collection '{collection_name}': {str(e)}")
            raise Exception(f"Failed to create collection: {str(e)}")
    
    async def list_collections(self, request: Optional[CollectionRequest] = None) -> list[str]:
        """
    ...
    async def delete_collection(self, collection_name: str, request: Optional[CollectionRequest] = None) -> bool:
        """
        ...
        """
        try:
            # Use default embeddings model if no request provided
            embeddings_model = request.embeddings_model if request else "nasa-impact/nasa-smd-ibm-st-v2"
            ...
            logger.error(f"ValueError deleting collection '{collection_name}': {str(e)}")
            raise
        except Exception as e:
