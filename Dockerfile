...
# torch is never requested directly: sentence-transformers pulls it in. Left alone, pip takes the
# default PyPI wheel, which bundles the CUDA runtime and weighs over 2 GB, on a service that runs on
# Fargate CPU and never sees a GPU. Installing the CPU build first means the transitive requirement
# is already satisfied when requirements.txt is resolved.
#
# This is the whole image: the v0.0.5 image was 3.0 GB compressed, of which a single layer, the
# virtualenv, was 2,957 MB. Every Fargate task start downloads that from ECR before the process
# begins, so it is paid on every deploy, every restart and every scale-out.
